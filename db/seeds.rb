...
user = User.create(username: "test_user_xyz", email: "testemail@email.com", password: "password", password_confirmation: "password", confirmed_at: Time.now)
byebug
UserWall.create(user_id: user.id)

description = "Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. Donec quam felis, ultricies nec, pellentesque eu, pretium quis, sem. Nulla consequat massa quis enim. Donec pede justo, fringilla vel, aliquet nec, vulputate eget, arcu. In enim justo, rhoncus ut, imperdiet a, venenatis vitae, justo. Nullam dictum."

list = List.create(title: "Random Browing Session", description: description, user_id: user.id)
...
urls.each do |url|
  if !last_created
    @link = Link.create(url: url, description: description[0..80], list_id: list.id)
  else
    @link = Link.create(url: url, description: description[0..80], link_id: last_created.id)
  end
  last_created = @link
end
